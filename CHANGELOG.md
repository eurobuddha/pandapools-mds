...
- **Stranding warnings (native 0.9.56).** Three levels derived from existing constants — notice past keep-fresh + 300, warning past the 1500-block discovery depth, urgent 50 blocks before the ~1700 cascade edge — posted through `MDS.notify` from the background service, because the users who need the warning are the ones whose app is not open. Escalation is immediate, an unchanged level waits a day, improvement never nags, and an unknown age is never escalated. A quarantined pool is told to **Withdraw**, not Re-publish: keep-fresh refuses to sign for it, so Re-publish cannot work. The keep-fresh pass now evaluates quarantined pools for stranding instead of skipping them — they are the ones that cannot self-heal.
- `Covenant.SENTINEL_SCAN_DEPTH` is now a real constant rather than `depth:1500` repeated inline; without it the warning thresholds silently evaluated against `undefined` and never fired.
- **The signing gate names which problem it is (native 0.9.49).** Six reasons — key absent, node unreadable, key exhausted, counter regressed, confirmation unsaved, signing quarantined — each with its own wording and the real numbers. The old single message said "paused **or** could not be verified", merging two cases that need opposite advice: a quarantined key wants a wallet restore; an unreadable reply wants the user to change nothing at all.
- **A backup is proven usable before it is handed over (native 0.9.51).** A MiniDapp cannot see the file the browser saves, so it verifies the JSON it produced — which still catches a backup that does not cover every pool you own, the failure you would otherwise discover during a real recovery. The dialog says what was and was not checked.
- 30 tests pass.
...
## [0.6.20] — Pool calculator: what happens to your pool when the price moves
- **Added** a what-if **pool calculator** (parity with native **0.9.30**): My LP → "Pool calculator", or "What if the price moves? ›" on any of your pool cards, which opens it seeded with that pool's live reserves. Enter a starting pool, then move the MINIMA price — type it, drag the log slider (÷10,000 … ×10,000) or tap a ÷100 … ×100 chip — and see the MINIMA and token in the pool, their ratio, the pool's value versus simply holding, the price move from entry, and the pool's point on its constant-product curve. Display only: nothing here touches the node or the chain.
- **Fees as a variable**: the swap fee rate (0.5 % by default — the covenant's) and the volume traded through the pool give the fees kept, K with fees (the "fee growth" figure), the reserves and value with fees folded in, and the volume needed to break even with holding at that price. Fees are valued at the current price, so pool value with fees = fee-free value + fees kept (√K′ = √K + fees ÷ 2√P); the dialog says so.
- New `calc.js` (`PoolCalc`: maths + curve drawing + the form itself; plain JS numbers — display only, the fund maths stays on Decimal in `curve.js`), byte-mirrored to Desktop `renderer/poolcalc.js`. Ships in the zip (`build.sh`).

## [0.6.19] — a swap no longer adopts the pool into your wallet balance
- **Fixed** the one-swap-pollutes-forever trap (parity with native **0.9.27**). Every swap ran `newscript trackall:true` on every routed pool — only so `txnbasics` could attach the covenant's ScriptProof — making the pool address permanently *relevant*, so a stranger's reserves counted into the swapper's `confirmed` balance forever. Nothing ever untracked it.
- **Swap path now registers foreign pools with `trackall:false`** (`ensureTrackedForSwap` in `poolmgr.js`): txnbasics reads the script table regardless of the track flag, so swaps build and post exactly as before, but the pool's coins never become relevant. A row already on the node — any track value — is left untouched, because `newscript` REPLACES an existing row and re-registering would downgrade an LP's own `trackall:true`. Owner flows (create/deposit/close/refresh/migrate/restore) still assert `trackall:true` — your own reserves are your capital.
- **Added a launch hygiene sweep** (`cleanForeign` in `service.js`, after `retrackOwn`): demotes every tracked covenant whose owner key is provably not this wallet's (`keys` + `pp_ownpools`; unreadable/empty reads → zero writes) back to `trackall:false` using the row's **verbatim** script, then `cointrack enable:false` on its lingering relevant coins. Full effect after the next node restart — core keeps a demoted address in its in-memory relevance cache until then, which is why the sweep re-runs every launch.
- **Fixed** the `retrackOwn` gate to check the row's **track flag**, not mere existence — a swap's `track:false` row must not block re-upgrading an own pool to `trackall:true`.
...
- Hardened after independent adversarial review: only the node's AFFIRMATIVE "not found" reply triggers the swap-path registration — an ambiguous failure writes nothing (`newscript` REPLACES rows, so a blind write could downgrade an LP's own row and fail-open); the sweep re-reads `pp_ownpools` immediately before writing so a Restore landing mid-sweep is never demoted; on a read-restricted install a pending-approval demote reply ABORTS the demote loop (no prompt storm, no stale pending downgrades); the coin pass runs one bounded `coins relevant:true address:` query per foreign address (the unbounded form can die at the 256K reply cap on exactly the polluted nodes this targets); `pp_ownpools` reads accept both column-name cases. Byte-mirrored with the desktop copy; verified by 33 mock-node sim assertions of this exact JS.
